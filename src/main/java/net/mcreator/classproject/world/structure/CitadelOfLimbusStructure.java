...
package net.mcreator.classproject.world.structure;
...
import net.mcreator.classproject.procedures.VirgilCitadelSpawnGenerationProcedure;
...
public class CitadelOfLimbusStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 1500) {
						...
						for (int a = 0; a < count; a++) {
							...
							BlockPos spawnTo = new BlockPos(i + -10, j + 0, k + -10);
							int x = spawnTo.getX();
							int y = spawnTo.getY();
							int z = spawnTo.getZ();
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("classproject", "floatingcitadel"));
							if (template == null)
								return false;
									...
									.addProcessor(BlockIgnoreStructureProcessor.AIR).setChunk(null).setIgnoreEntities(false), random);
							{
								...
								VirgilCitadelSpawnGenerationProcedure.executeProcedure($_dependencies);
							}
						}
					}
					return true;
			...
			event.getRegistry().register(feature.setRegistryName("citadel_of_limbus"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("classproject:citadel_of_limbus"), configuredFeature);
		}
	}
	@SubscribeEvent
